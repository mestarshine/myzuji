...
/**
 * An {@link ExecutorService} for running {@link MyForkJoinTask}s.
 ...
 * <td> {@link #execute(MyForkJoinTask)}</td>
 * <td> {@link MyForkJoinTask#fork}</td>
 * </tr>
 * <tr>
 * <td> <b>Await and obtain result</b></td>
 * <td> {@link #invoke(MyForkJoinTask)}</td>
 * <td> {@link MyForkJoinTask#invoke}</td>
 * </tr>
 * <tr>
 * <td> <b>Arrange exec and obtain Future</b></td>
 * <td> {@link #submit(MyForkJoinTask)}</td>
 * <td> {@link MyForkJoinTask#fork} (ForkJoinTasks <em>are</em> Futures)</td>
 * </tr>
 * </table>
 *
 ...
 */
@sun.misc.Contended
public class MyForkJoinPool extends AbstractExecutorService {
     ...
     */
    static final MyForkJoinPool common;
    /**
     ...
     */
    private static final int commonMaxSpares;
    ...
    static {
        ...
        common = java.security.AccessController.doPrivileged
            (new java.security.PrivilegedAction<MyForkJoinPool>() {
                @Override
                public MyForkJoinPool run() {
     ...
     * makeCommonPool.
     */
    private MyForkJoinPool(int parallelism,
                           ForkJoinWorkerThreadFactory factory,
                           UncaughtExceptionHandler handler,
                           int mode,
    ...
    static int getSurplusQueuedTaskCount() {
        ...
        if (((t = Thread.currentThread()) instanceof MyForkJoinWorkerThread)) {
            int p = (pool = (wt = (MyForkJoinWorkerThread) t).pool).
                config & SMASK;
    ...
    static WorkQueue commonSubmitterQueue() {
        MyForkJoinPool p = common;
        int r = MyThreadLocalRandom.getProbe();
        WorkQueue[] ws;
        int m;
     ...
     * MyForkJoinTask#inForkJoinPool() running in a ForkJoinPool}, this
    ...
    public static void managedBlock(ManagedBlocker blocker)
        ...
        if ((t instanceof MyForkJoinWorkerThread) &&
            (p = (wt = (MyForkJoinWorkerThread) t).pool) != null) {
     ...
     */
    private static MyForkJoinPool makeCommonPool() {
        int parallelism = -1;
        ...
        return new MyForkJoinPool(parallelism, factory, handler, LIFO_QUEUE,
            "ForkJoinPool.commonPool-worker-");
    }

    /**
     * Acquires the runState lock; returns current (locked) runState.<br/>
     * CAS 上锁
     */
    private int lockRunState() {
        int rs;
        // 返回的 rs 都是携带了锁标识的
        return ((((rs = runState) & RSLOCK) != 0 ||   //已经有锁
            !U.compareAndSwapInt(this, RUNSTATE, rs, rs |= RSLOCK)) ?   // 没有线程获取锁，那么 CAS 获取
            awaitRunStateLock() : rs);
    }

    /**
     * Spins and/or blocks until runstate lock is available.  See
     * above for explanation.<br/>
     * 等待获取锁
     */
    private int awaitRunStateLock() {
        Object lock;
        boolean wasInterrupted = false;
        for (int spins = SPINS, r = 0, rs, ns; ; ) {  // 自旋锁，自旋次数位0
            // 无锁状态，锁已经被释放，那么可以去 CAS 竞争锁
            ...
            //初始化随机数
                ...
                r = MyThreadLocalRandom.nextSecondarySeed();
            ...
            // STARTED 表明 workqueue 已经被初始化了
            else if ((rs & STARTED) == 0 || (lock = stealCounter) == null) {
                Thread.yield();   // 由于当前 rs 的 STARTED 状态位为 0 ，代表当前 FJP 没有在运行了，那么没有必要再去睡眠了，因为这个状态维持时间非常短，线程竞争激烈，那么释放当前 CPU
            } else if (U.compareAndSwapInt(this, RUNSTATE, rs, rs | RSIGNAL)) {  // 光是睡眠不行，需要有人唤醒，所以这里必须置 RSIGNAL 标志位，提示另外的线程需要唤醒它
                synchronized (lock) {
                    if ((runState & RSIGNAL) != 0) { // rs 置位成功
                        try {
                            lock.wait();
                        } catch (InterruptedException ie) {
                            if (!(Thread.currentThread() instanceof
                                MyForkJoinWorkerThread)) {
                                wasInterrupted = true;
                            }
                        }
                    } else {
    ...
    /**
     * Unlocks and sets runState to newRunState. <br/>
     * 解锁
     *
    ...
    private boolean createWorker() {
        ...
        Throwable ex = null;
        MyForkJoinWorkerThread wt = null;
        try {
     ...
     */
    final WorkQueue registerWorker(MyForkJoinWorkerThread wt) {
        UncaughtExceptionHandler handler;
     ...
     */
    final void deregisterWorker(MyForkJoinWorkerThread wt, Throwable ex) {
        WorkQueue w = null;
        ...
        {
            MyForkJoinTask.helpExpungeStaleExceptions();
        } else                                          // rethrow
        {
            MyForkJoinTask.rethrow(ex);
        }
    }
     ...
     */
    private MyForkJoinTask<?> scan(WorkQueue w, int r) {
        WorkQueue[] ws;
        int m;
     ...
     */
    final int helpComplete(WorkQueue w, MyCountedCompleter<?> task,
                           int maxTasks) {
        WorkQueue[] ws;
        int s = 0, m;
     ...
     */
    private void helpStealer(WorkQueue w, MyForkJoinTask<?> task) {
            ...
            do {                                       // restart point
                ...
                for (subtask = task; subtask.status >= 0; ) {
                    ...
                    for (; ; ) {                         // help v or descend
                        ...
                        int b;
                        checkSum += (b = v.base);
                        MyForkJoinTask<?> next = v.currentJoin;
                        ...
                        MyForkJoinTask<?> t = ((MyForkJoinTask<?>)
                        ...
                        if (v.base == b) {
                            ...
                            if (U.compareAndSwapObject(a, i, t, null)) {
                                v.base = b + 1;
                                MyForkJoinTask<?> ps = w.currentSteal;
                                int top = w.top;
                                do {
     ...
     * @return task status on exit
     */
    final int awaitJoin(WorkQueue w, MyForkJoinTask<?> task, long deadline) {
        int s = 0;
        if (task != null && w != null) {
            MyForkJoinTask<?> prevJoin = w.currentJoin;
            U.putOrderedObject(w, QCURRENTJOIN, task);
            MyCountedCompleter<?> cc = (task instanceof MyCountedCompleter) ?
                (MyCountedCompleter<?>) task : null;
            for (; ; ) {
                if ((s = task.status) < 0) {
                    break;
                }
    ...
    private WorkQueue findNonEmptyStealQueue() {
        ...
        int r = MyThreadLocalRandom.nextSecondarySeed();
        if ((ws = workQueues) != null && (m = ws.length - 1) >= 0) {
            for (int origin = r & m, k = origin, oldSum = 0, checkSum = 0; ; ) {
                WorkQueue q;
                int b;
    ...
    final void helpQuiescePool(WorkQueue w) {
        MyForkJoinTask<?> ps = w.currentSteal; // save context
     ...
     */
    final MyForkJoinTask<?> nextTaskFor(WorkQueue w) {
        for (MyForkJoinTask<?> t; ; ) {
            WorkQueue q;
            int b;
     ...
     * queue if the one at index if empty or contended.<br/>
     * 真正执行提交任务的方法
     *
     * @param task the task. Caller must ensure non-null.
     */
    private void externalSubmit(MyForkJoinTask<?> task) {
        // 随机数
        int r;
        // 取随机数
        if ((r = MyThreadLocalRandom.getProbe()) == 0) {
            MyThreadLocalRandom.localInit();
            r = MyThreadLocalRandom.getProbe();
        }
        // 如果出现这种死循环，必然会有多个判断条件，多个执行分支
        // 1、初始化全局 workQueue
        // 2、根据随机数种子，分发到外部提交队列
        //      2.1、如果外部提交队列为空，则初始化
        for (; ; ) {
            ...
            if ((rs = runState) < 0) {
                ...
                ((ws = workQueues) == null ||  // workQueues 需要初始化
                    (m = ws.length - 1) < 0)) {
                // 注意：如果该分支不执行，但是由于｜｜ 运算符的存在，这里 ws 和 m 变量已经初始化
                int ns = 0;
                rs = lockRunState();
                try {
                    if ((rs & STARTED) == 0) { // 因为多线程操作，可能上面判断后，已经被别的线程初始化了，所以没必要再进行 CAS
                        U.compareAndSwapObject(this, STEALCOUNTER, null,
                            new AtomicLong());
                        // create workQueues array with size a power of two
                        // p 就是并行度：工作线程的数量, 至少有4个 slots
                        ...
                        ns = STARTED; // 初始化成功，那么此时状态修为 STARTED 的状态
                    }
                } finally {
                    // 最后释放锁
            ...
            // 创建一个新的 workqueue 并判断 rslock，如果别的线程拿到锁，则表示 该 workqueue 太忙，重新获取随机数，进行分发
            ...
            if (move) {
                r = MyThreadLocalRandom.advanceProbe(r);
            }
        }
    }
     ...
     * for externalSubmit. <br/>
     * 此方法取随机数种子
     *
     * @param task the task. Caller must ensure non-null.
     */
    final void externalPush(MyForkJoinTask<?> task) {
        WorkQueue[] ws;
        WorkQueue q;
        int m;
        // 取 TLR 的随机数种子
        int r = MyThreadLocalRandom.getProbe();
        int rs = runState; // 当前 FJP 的运行状态
        if ((ws = workQueues) != null && (m = (ws.length - 1)) >= 0 &&
     ...
     */
    final boolean tryExternalUnpush(MyForkJoinTask<?> task) {
        WorkQueue[] ws;
        WorkQueue w;
        MyForkJoinTask<?>[] a;
        int m, s;
     ...
     */
    final int externalHelpComplete(MyCountedCompleter<?> task, int maxTasks) {
        WorkQueue[] ws;
        int n;
        int r = MyThreadLocalRandom.getProbe();
     ...
     *                                    scheduled for execution
     */
    public void execute(MyForkJoinTask<?> task) {
        if (task == null) {
            throw new NullPointerException();
        }
        externalPush(task);
    ...
    public void execute(Runnable task) {
        ...
        MyForkJoinTask<?> job;
        if (task instanceof MyForkJoinTask<?>) // avoid re-wrap
        {
            job = (MyForkJoinTask<?>) task;
        } else {
            job = new MyForkJoinTask.RunnableExecuteAction(task);
        }
        externalPush(job);
    }
     ...
     *                                    scheduled for execution
     */
    public <T> MyForkJoinTask<T> submit(MyForkJoinTask<T> task) {
        if (task == null) {
            throw new NullPointerException();
        }
        externalPush(task);
     ...
     */
    @Override
    public <T> MyForkJoinTask<T> submit(Callable<T> task) {
        MyForkJoinTask<T> job = new MyForkJoinTask.AdaptedCallable<T>(task);
        externalPush(job);
        return job;
    }
     ...
     */
    @Override
    public <T> MyForkJoinTask<T> submit(Runnable task, T result) {
        MyForkJoinTask<T> job = new MyForkJoinTask.AdaptedRunnable<T>(task, result);
        externalPush(job);
        return job;
    }
     ...
     */
    @Override
    public MyForkJoinTask<?> submit(Runnable task) {
        if (task == null) {
            throw new NullPointerException();
        }
        MyForkJoinTask<?> job;
        if (task instanceof MyForkJoinTask<?>) // avoid re-wrap
        {
            ...
            // 将任务封装为FJT，因为 FJP 只执行 FJT 的任务
            job = new MyForkJoinTask.AdaptedRunnableAction(task);
        }
        // 核心提交任务方法
        externalPush(job);
        return job;
    ...
    public <T> List<Future<T>> invokeAll(Collection<? extends Callable<T>> tasks) {
        ...
        try {
            for (Callable<T> t : tasks) {
                MyForkJoinTask<T> f = new MyForkJoinTask.AdaptedCallable<T>(t);
                futures.add(f);
                externalPush(f);
            }
            for (Future<T> future : futures) {
                ((MyForkJoinTask<?>) future).quietlyJoin();
            }
            done = true;
            return futures;
        } finally {
     ...
     */
    protected MyForkJoinTask<?> pollSubmission() {
        WorkQueue[] ws;
        WorkQueue w;
        MyForkJoinTask<?> t;
        if ((ws = workQueues) != null) {
     ...
     */
    protected int drainTasksTo(Collection<? super MyForkJoinTask<?>> c) {
        int count = 0;
        WorkQueue[] ws;
        WorkQueue w;
        MyForkJoinTask<?> t;
     ...
     * advisory notes for class {@link MyForkJoinTask} stating that
     ...
     * in effect to {@link MyForkJoinTask#helpQuiesce}. Otherwise,
    ...
    public boolean awaitQuiescence(long timeout, TimeUnit unit) {
        ...
        if ((thread instanceof MyForkJoinWorkerThread) &&
            (wt = (MyForkJoinWorkerThread) thread).pool == this) {
    ...
    protected <T> RunnableFuture<T> newTaskFor(Runnable runnable, T value) {
        return new MyForkJoinTask.AdaptedRunnable<T>(runnable, value);
    }

    @Override
    protected <T> RunnableFuture<T> newTaskFor(Callable<T> callable) {
        return new MyForkJoinTask.AdaptedCallable<T>(callable);
    }

    /**
     * Factory for creating new {@link MyForkJoinWorkerThread}s.
    ...
    public interface ForkJoinWorkerThreadFactory {
         ...
         */
        MyForkJoinWorkerThread newThread(MyForkJoinPool pool);
    }

    /**
     * Interface for extending managed parallelism for tasks running
     * in {@link MyForkJoinPool}s.
     *
     ...
     * thread invoking {@link MyForkJoinPool#managedBlock(ManagedBlocker)}.
     ...
     */
    static final class EmptyTask extends MyForkJoinTask<Void> {
        private static final long serialVersionUID = -7721805057305804111L;

        EmptyTask() {
            status = MyForkJoinTask.NORMAL;
        } // force done
    ...
    static final class WorkQueue {
        ...
        final MyForkJoinPool pool;   // the containing pool (may be null)
        final MyForkJoinWorkerThread owner; // owning thread or null if shared
        // Instance fields
        ...
        MyForkJoinTask<?>[] array;   // the elements (initially unallocated)
        volatile Thread parker;    // == owner during call to park; else null
        volatile MyForkJoinTask<?> currentJoin;  // task being joined in awaitJoin
        volatile MyForkJoinTask<?> currentSteal; // mainly used by helpStealer

        WorkQueue(MyForkJoinPool pool, MyForkJoinWorkerThread owner) {
            this.pool = pool;
            this.owner = owner;
        ...
        final MyForkJoinTask<?>[] growArray() {
            ...
            int oldMask, t, b;
            MyForkJoinTask<?>[] a = array = new MyForkJoinTask<?>[size];
            if (oldA != null && (oldMask = oldA.length - 1) >= 0 &&
                ...
                do { // emulate poll from old array, push to new array
                    ...
                    x = (MyForkJoinTask<?>) U.getObjectVolatile(oldA, oldj);
                    if (x != null &&
        ...
        final MyForkJoinTask<?> pollAt(int b) {
            ...
            if ((a = array) != null) {
                ...
                if ((t = (MyForkJoinTask<?>) U.getObjectVolatile(a, j)) != null &&
        ...
        final MyForkJoinTask<?> poll() {
            ...
            while ((b = base) - top < 0 && (a = array) != null) {
                int j = (((a.length - 1) & b) << ASHIFT) + ABASE;
                t = (MyForkJoinTask<?>) U.getObjectVolatile(a, j);
                if (base == b) {
                    if (t != null) {
                        if (U.compareAndSwapObject(a, j, t, null)) {
                            base = b + 1;
        ...
        final MyForkJoinTask<?> peek() {
            ...
            return (MyForkJoinTask<?>) U.getObjectVolatile(a, j);
        }
        ...
        final void cancelAll() {
            ...
            if ((t = currentJoin) != null) {
                currentJoin = null;
                MyForkJoinTask.cancelIgnoringExceptions(t);
            }
        ...
        final void runTask(MyForkJoinTask<?> task) {
            if (task != null) {
                ...
                execLocalTasks();
                MyForkJoinWorkerThread thread = owner;
        ...
        public final MyForkJoinWorkerThread newThread(MyForkJoinPool pool) {
            return java.security.AccessController.doPrivileged(
                new java.security.PrivilegedAction<MyForkJoinWorkerThread>() {
                    @Override
                    public MyForkJoinWorkerThread run() {
                        return new MyForkJoinWorkerThread.InnocuousMyForkJoinWorkerThread(pool);
                    }
                }, innocuousAcc);
        }
    }

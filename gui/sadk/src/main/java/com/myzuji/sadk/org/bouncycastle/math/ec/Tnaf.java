...
public class Tnaf {
    ...
    public static ZTauElement round(SimpleBigDecimal lambda0, SimpleBigDecimal lambda1, byte mu) {
        ...
        if (lambda1.getScale() != scale) {
            ...
            BigInteger q0 = f0.add(BigInteger.valueOf((long) h0));
            BigInteger q1 = f1.add(BigInteger.valueOf((long) h1));
    ...
    public static byte[] tauAdicNaf(byte mu, ZTauElement lambda) {
        if (mu != 1 && mu != -1) {
            ...
            for (BigInteger r1 = lambda.v; !r0.equals(ECConstants.ZERO) || !r1.equals(ECConstants.ZERO); ++i) {
                if (r0.testBit(0)) {
    ...
    public static byte getMu(ECCurve.F2m curve) {
        if (!curve.isKoblitz()) {
            ...
            return (byte) (curve.getA().isZero() ? -1 : 1);
        }
    }
    ...
    public static byte[] tauAdicWNaf(byte mu, ZTauElement lambda, byte width, BigInteger pow2w, BigInteger tw, ZTauElement[] alpha) {
        if (mu != 1 && mu != -1) {
            ...
            for (int i = 0; !r0.equals(ECConstants.ZERO) || !r1.equals(ECConstants.ZERO); ++i) {
                BigInteger t;
                if (r0.testBit(0)) {
                    t = r0.add(r1.multiply(tw)).mod(pow2w);
                    byte uLocal;
                    if (t.compareTo(pow2wMin1) >= 0) {
                        uLocal = (byte) t.subtract(pow2w).intValue();
                    } else {
                        uLocal = (byte) t.intValue();
                    }

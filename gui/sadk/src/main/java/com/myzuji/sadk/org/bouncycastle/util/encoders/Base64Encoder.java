...
public class Base64Encoder implements Encoder {
    ...
    protected final byte[] fastQueryTable = new byte[256];
    ...
    private final void initialiseFastQueryTable() {
        for (int i = 0; i < this.fastQueryTable.length; ++i) {
            this.fastQueryTable[i] = -1;
        }

        for (int i = 0; i < this.encodingTable.length; ++i) {
            this.fastQueryTable[this.encodingTable[i]] = 1;
        }

        byte[] others = new byte[]{61, 32, 13, 10, 9};

        for (int i = 0; i < others.length; ++i) {
            this.fastQueryTable[others[i]] = 1;
        }

    }

    protected final boolean isBase64Compatability(byte[] data) {
        if (data != null && data.length != 0) {
            boolean isBase64Compatibility = true;

            for (int i = 0; i < data.length; ++i) {
                if (data[i] < 0 || this.fastQueryTable[data[i]] == -1) {
            ...
            throw new IllegalArgumentException("data required not be null");
        }
    }
}
